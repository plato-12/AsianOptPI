...
This vignette demonstrates practical applications of the `AsianOptPI` package
...
factors <- compute_effective_factors(u = 1.2, d = 0.8, lambda = 0.1, v_u = 1, v_d = 1)
p_eff <- compute_p_eff(r = 1.05, u = 1.2, d = 0.8, lambda = 0.1, v_u = 1, v_d = 1)

cat("Effective Factors:\n")
cat("  u_tilde:", round(factors$u_tilde, 4), "\n")
cat("  d_tilde:", round(factors$d_tilde, 4), "\n")
cat("  p_eff:  ", round(p_eff, 4), "\n\n")

# Verify no-arbitrage
cat("No-arbitrage check: d̃ < r < ũ\n")
cat("  d̃ =", round(factors$d_tilde, 4), "\n")
cat("  r  =", 1.05, "\n")
cat("  ũ =", round(factors$u_tilde, 4), "\n")
...
```{r}
# Compute prices for increasing n (up to feasible limit)
n_seq <- c(1, 2, 3, 5, 7, 10, 12, 15)
prices <- numeric(length(n_seq))

for (i in seq_along(n_seq)) {
  prices[i] <- price_geometric_asian(100, 100, 1.05, 1.2, 0.8, 0.1, 1, 1, n_seq[i])
}

plot(n_seq, prices, type = "b",
     xlab = "Number of Time Steps (n)",
     ylab = "Option Price",
     main = "Convergence of Option Price with Increasing n",
     col = "darkblue", pch = 19)
grid()
abline(h = tail(prices, 1), lty = 2, col = "gray")
```
