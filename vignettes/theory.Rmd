...
This vignette provides the mathematical foundation for pricing Asian options
with price impact using the Cox-Ross-Rubinstein (CRR) binomial model.
...
When market makers hedge options, they must buy or sell the underlying stock.
Large volumes cause price movements:
...
The effective risk-neutral probability becomes:

$$p^{eff} = \frac{r - \tilde{d}}{\tilde{u} - \tilde{d}}$$

## Replicating Portfolio Method

### Fundamental Principle

At any node, we replicate the option using:
...
Note: UD and DU have the same terminal price but **different geometric averages**.
